...
## Network (Base mainnet — V5.1 deploy)
...
| RPC URL (default) | `https://mainnet.base.org` (overridable via `NEXT_PUBLIC_RPC_URL`) |
| Explorer | https://basescan.org |
| Native gas token | ETH (Base mainnet) |
| USDC token (USDC) | `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913` |

## Auto-switch behavior

If your wallet is connected to the wrong chain (anything other than 8453), the operate-app shows a red banner with a one-click **"Switch to Base mainnet"** button. The button calls `useSwitchChain({ chainId: base.id })` from wagmi.

If your wallet has no Base mainnet configured, the switch attempt prompts the wallet to add the network.

## Get test USDC

Lumina uses **USDC** on Base mainnet. To request test tokens for your wallet, contact `labs@lumina-org.com`. Public faucet endpoint pending.
...
| Problem | Fix |
|---|---|
| "Wrong network" banner stuck | Click "Switch to Base mainnet" — if it fails, switch manually in MetaMask → Networks → Add Network → use values above |
...
- Frontend provider: [`components/lumina/web3-provider.tsx`](https://github.com/org-lumina/v0-lumina-landing-page/blob/main/components/lumina/web3-provider.tsx) (`createConfig` line 61, `chains: [base]` line 63)
- Chain config: [`wagmi/chains` `base`](https://github.com/wevm/wagmi/blob/main/packages/core/src/chains.ts)
